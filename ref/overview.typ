...
#title-page[
  ...
  依次有四种*展示方式*：「倍数系」侧重号数之间的二倍关系，「映射表」侧重每一号数对应几点，「点数图」侧重有哪些点数被号数覆盖，「分支树」侧重数据源间的相近程度。前三者相对客观，而分支树十分主观，请酌情参考。另外注意尽管后三者都会列出数据源，但为适应不同展示方式，列出的顺序不完全相同。

  每个*数据源*都有*代号*，例如@source:基准、@source:方正书版、@source:王选1982 等。代号只是为了方便沟通，命名方式并不统一。具体含义可单击绿色文字跳转到后页查看。

  绘制点数等图时，会将*号数*分为两个*系列*：「□号系列」包括初号、一号、二号……和特大、特中、特初、特号、大特、小特，而「小□系列」包括小初、小一、小二……和七行、六行、五行、四行与七倍、六倍、五倍。这种分法并没有特殊道理，只是让绘出的图方便比较。
]
#set heading(offset: 1)

#pagebreak()
#include "overview-sankey.typ"
