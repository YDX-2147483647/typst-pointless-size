  ...
  grid(
    ...
    ..for images in rows.pos() {
      ...
      let columns = images.map(img => {
        // 必须给定 width，否则不支持 image(…, width: 60%)；给定的具体值则无所谓
        let (height, width) = measure(img, width: 100pt)
        width / height * 1fr
      })
...
#let sources = (
  基准: (
    ...
    notes: [
      ...
      - *MS Word*#footnote[仅限中文版 MS Word。按 @张小衡2006 的说法，英文版 MS Word 不支持字号，并且如果先用中文版设置号数，再用英文版打开，那么设置中会显示点数。另外，当前微软的 PowerPoint、Excel、画图均无号数可选，苹果 iCloud 上的 Pages 也无号数可选。]
        ...
        - 香港某大学 @张小衡2006 测试了 Windows 上的 MS Word 2003，发现数值、定义范围与基准完全相同#footnote[不过该文行文很不靠谱，还说 PowerPoint、Excel 也适用，但当今 PowerPoint、Excel 并不支持号数制。]。
  ...
  老CCT: (
    ...
    notes: [
      ...
      `cct.dat`文件内容如下#footnote[其中有一处space错拼为sapce，原文如此。]。
  ...
  政府: (
    ...
    notes: [
      ...
      又，#link("https://std.samr.gov.cn/gb/search/gbDetailed?id=71F772D78446D3A7E05397BE0A0AB82A")[GB/T 12200.2—1994《汉语信息处理词汇　02部分：汉语和汉字》]「4 术语和定义 → 4.1 汉语和汉字 → 4.1.2 文字 → 4.1.2.9 字号 character number」展示了字号大小，不过未定义点数，且定义范围只有初号到六号与小初、小四、小五。另外，小四、小五括注了「新四号」「新五号」，但小初没有，有些蹊跷。
      ...
      手工测量「国」字框纵向所跨像素数，比例大约是 $35.6 : 31.5 : 27.2 : 21.2 : 16.1 : 13.9 : 12.0 : 10.5 : 9.59 : 8.67$。若假设五号是 $10.5 pt$ 并按比例推算，则结果如下图。
      #draw(
        ```csv
        初号,35.6
        小初,31.5
  ...
  叶重光1996: (
    ...
    notes: [
      ...
      #quote(attribution: [69–70页（重新分过段，但未改动文字及标点符号）])[
        ...
        - 如将标准汉字用缩去 $1/3$ 保留 $2/3$ 的办法，头号字#footnote[前文称「1号」，此处及后文称「头号」，原文如此。]可缩为标准新2号字，新1号字可缩为标准的3号字，3号字可缩为标准的5号字，4号字可缩为标准的新5号字，新4号字可缩为标准的6号字，6号字可缩为标准的7号字。用缩去 $1/3$ 的办法，可将现成的标准汉字缩出六个比原字小的标准小号汉字；
  ...
  小史1981: (
    ...
    notes: [
      ...
      表2记录了正方、狭长、扁宽三种字身形式的活字，这里只记录正方#footnote[狭长、扁宽的号数覆盖范围小于正方，而且纵向点数与正方一致，唯一例外是狭长小初号有 $31.5 times 24$ 与 $30 times 24$ 两种规格，前者的纵向点数与正方小初号 $30$ 不同。]。另外，表2中初号与二号之间是「大号」，但正文一直称「一号」。
  ...
  王选1982: (
    ...
    notes: [
      ...
      - 460页例1提供了初号、头号与二、三、四、小四、五、小五、六、七号的示例，并说系统还允许特大号、特号、小特号、小初号、小二号、小六号。定义范围与《计算机学报》一致，不过一号被称作「头号」。
  ...
  东京1942: (
    ...
    notes: [
      ...
      此书有四百多页，其中以下几节相关。第一编、第二编的作者分别是宮崎榮太郎、飯田常治，不是同一人，所以两部分不完全搭配。
      #[
  ...
  东京1931: (
    ...
    notes: [
      ...
      #quote(attribution: [13–14页（frame 13–14页）])[
        *活字の尺度を現はす單位*
        #h(1em, weak: true)
        活字の大きさは柄の深さを以て測るのである。我国在来の活字は、本木昌造が西洋活字を輸入するに當りてその大きさを制定する必要を生じ、左の八種を常用活字とした。

        #figure({
          let space = hide[字の切口一邊約]
          table(
            columns: 4,
            align: start,
            column-gutter: (1em, 5em, 1em),
            [初號], [活字の切口一邊約 $44/72$ 吋], [四號], [同#space $14/72$],
            [一號], [同#space $28/72$], [五號], [同#space $11/72$],
            [二號], [同#space $22/72$], [六號], [同#space $8/72$],
            [三號], [同#space $16/72$], [七號], [同#space $5.5/72$],
          )
        })

        右のうち五號活字を以てその標準とした。五號は西洋のスモールパイカを基準として $11/72$ 吋に作ったものであるが、今ではそれよりも小さくなつて $10.5/72$ 吋程度となったのみならず、新たな鋳造所が各所に出來たためにその均整を缺き、少からず不便を來たした。そこでアメリカのポイント式が採用せらるゝに至つた。

        #divider()
        *表示活字尺度的单位*
        #h(1em, weak: true)
        活字的大小是以柄的深度来测量的。我国传统的活字，在本木昌造输入西洋活字之际，产生了制定其大小的必要，将以下八种作为常用活字。

        #align(center)[（表格略）]

        右侧#footnote[原文竖排，所以右侧相当于上文。]以五号活字为标准。五号是以西洋的 small pica 为基准，制成 $11/72$ 吋的东西，但如今比那变得更小了，成为 $10.5/72$ 吋的程度，不仅如此，新的铸造厂在各处出现，因为其匀整缺失，带来了不少不便。因此，美国的point式被采用而至。
      ]
      #quote(attribution: [16页（frame 15页）])[
        *主要活字*
        #h(1em, weak: true)
        活字の大きさは次に示す如き種類があるが、書籍雑誌の本文に最も多く使用されるものは五號と九ポイントである。この頃では、五號は少し大き過ぎる傾向があるので九ポイントがより多く採用されるやうになつた。婦人雜誌などは更に小型の八ポイント乃至は七・五ポイントの活字を用ひてゐる。標題に用ひられる活字は版面の體裁等を考慮した上で、四、三、二、一、初號等である。

        #divider()
        *主要活字*
        #h(1em, weak: true)
        活字的大小有如下所示的种类，但书籍杂志的正文中使用最多的是五号和九point。如今，五号有稍大过度的倾向，所以九point变得被更多地采用。妇女杂志等使用更小型的八point乃至七·五point的活字。标题中使用的活字是在考虑版面体裁等之上，使用四、三、二、一、初号等。
      ]
      #quote(attribution: [13–21页（frame 13–17页）各小节的标题])[
        #show: columns.with(2)
        #show divider: it => {
          place(horizon + right, dx: -1em, line(angle: 90deg, length: 10em, stroke: 0.05em))
          colbreak()
        }
      ...
      18–19页（frame 16页）两种点数制系统总结如下图。
      #draw(
        (42, 36, 30, 27, 24, 21, 18, 15, 12, 9, 6).map(n => "," + str(n) + "\n").join(),
      )
      #draw(
        (35, 28, 21, 14, 10.5, 7, 3.5).map(n => "," + str(n) + "\n").join(),
      )
      #figure(text(dir: rtl, grid(
        columns: (1fr,) * 4,
        gutter: 0.5em,
        ..range(2).map(n => image("assets/东京1931-节选.pdf", page: n + 1)),
        ..range(2, 8).map(n => grid.cell(colspan: 2, image("assets/东京1931-节选.pdf", page: n + 1))),
      )))
    ],
  ),
  秀英1903: (
  ...
  沪一厂1988: (
    ...
    notes: [
      ...
      此文件将一号、小一分别称作「大号」「小大号」。已知页面中只有11页扁的隶书体有「小特号 42×36点」，这里记录其纵向点数 $42 pt$。
  ...
  长新厂1973: (
    ...
    notes: [
      ...
      表下注「每点为0.35毫米」，而「点数」「毫米」两列也确实严格符合 $1 pt = 350 "μm"$。

      「我厂现有规格」非空的几行都接近「毫米」一列，相差不超过 $0.08 "mm"$。若把「我厂现有规格」按 $1 pt = 350 "μm"$ 换算回点数，则近似如下图。
      #figure(draw-as-log-period(
        (
          初号: 14.76,
          小初: 12.64,
          一号: 9.68,
          ...
          .pairs()
          .map(((g, p)) => (g, calc.round(p / 0.35, digits: 2))),
        width: 28em,
        height: 12em,
      ))
      在这本小手册的样本页，「点、薄（P）」一栏均与规格页「点数」相同，「忽米（道）#footnote[
        据此手册末页「统一公制计量单位名称」，忽米代号 $"cmm"$，意为十万分之一米。换句话说，$1 "cmm" = 10 "μm"$。
      ]」一栏未标注释的（例如二号）均与规格页「我厂现有规格」无矛盾（数值基本一致，但有效数字位数不全相同），而标了注释的则比较混乱——有的是注释内「本厂现用死面规格为……」更接近规格页「我厂现有规格」（例如一号在样本页写 $948 "cmm"$，注现用 $968 "cmm"$，而在规格页写 $9.625 "mm"$，现用 $9.68 "mm"$，四数中二、四两个数相等），有的却是被注释者更接近规格页「我厂现有规格」（例如四号在样本页写 $474 "cmm"$，注现用 $484 "cmm"$，而在规格页写 $4.8125 "mm"$，现用 $4.74 "mm"$，四数中一、四两个数相等）。这些细节涉及页数较多，以下只能展示几页；详细情况可访问 Internet Archive 查看。

      #figure({
        let page(p) = image("assets/长新厂1973-节选.pdf", page: p)
        context grid-fit-columns(
          (1, 2, 4).map(page),
          (3, 6).map(page),
          (5, 7).map(page),
  ...
  黔灵厂1963: (
    ...
    notes: [
      ...
      #figure({
        let page(p) = image("assets/黔灵厂1963-节选.pdf", page: p)
        context grid-fit-columns(
          (1, 2).map(page),
          (3, 4).map(page),
        )
  ...
  常州厂1983: (
    ...
    notes: [
      ...
      #figure(context grid-fit-columns(
        ..range(6).map(p => image("assets/常州厂1983-节选.pdf", page: p + 1)).chunks(2),
        (image("assets/常州厂1983-末页-增强图像.jpg", width: 80%),),
      ))

      另外，根据以下孔夫子旧書网#link("https://book.kongfz.com/27072/10382904116")[《常州市中国民族文字模厂 字模临时样张》]（江苏省无锡市壮壮古旧书店，2026年8月18日上书，售价￥190.00，说明页落款1987年10月），常州市中国民族文字模厂筹办于1982年，且同样位于卢家巷。那么常州武进县卢家巷字模厂很可能是这个厂创建初期的临时名称。孔夫子旧書网上也有许多常州市中国民族文字模厂的字模样本，汉字、西文与维、哈、柯文都称号数，不过未见号数与点数的映射关系。
      #figure(context grid-fit-columns(
        range(3).map(p => image("assets/常州厂1987-节选.pdf", page: p + 1)),
      ))
      又，常州华文也位于卢家巷，网传常州市中国民族文字模厂与常州华文联系密切。
    ],
  ),
  津报厂1971: (
    brief: [方体字各栏容纳字数表，#link("https://book.kongfz.com/252081/6194777990")[天津日報《各种铅字花边样本》]，1971年12月#footnote[
    ...
    notes: [
      ...
      #quote(attribution: [说明页])[
        “镑”（Point，简写Pt.或P.不是重量）和“点”是相同的，它们都是一英吋的七十二分之一。一“镑”即一“点”，七十二“镑”为一英吋。如八“镑”字九个的长度或九“镑”字八个的长度均为一英吋，折合公制约25.4毫米。
      ]
      #figure({
        let page(p, ..args) = image("assets/津报厂1971-节选.pdf", page: p, ..args)
        context grid-fit-columns(
          (1, 2).map(page),
          (page(3, width: 80%),),
        )
  ...
  津报厂1987: (
    ...
    notes: [
      ...
      另外虽然表中写一号 $27.5 pt$，但12页样本写一号 $28 pt$。此处按前者记录。

      #figure({
        let page(p, ..args) = image("assets/津报厂1987-节选.pdf", page: p, ..args)
        context grid-fit-columns(
          (1, 3).map(page),
          (page(2, width: 50%),),
        )
  ...
  重庆厂1980: (
    ...
    notes: [
      ...
      #figure({
        let page(p, ..args) = image("assets/重庆厂1980-节选.pdf", page: p, ..args)
        context grid-fit-columns(
          ..range(1, 9).map(page).chunks(2),
          (page(9, width: 80%),),
        )
      })
    ],
  ...
  王益1946: (
    ...
    notes: [
      ...
      此书所说的「體子」是指字体。此书正文称一号为「頭號」，但12页（PDF 14页）「用幾號字」插图称一号为「大號」。
  ...
  何继曾1959: (
    ...
    notes: [
      ...
      此书比较特殊的是七号分大小两种，二者线度分别是四号、五号的一半。又，13页（PDF 17页）表1「字盘每格的容量」中只有一号至六号，下注「七号字一般无定量，視需要情形而配置」。
  ...
  科学1978: (
    ...
    notes: [
      ...
      此处主要记录表格本体的号数与点数；对于标 \* 的点数，记录注释 \* 中「精确計算……应合……点」的点数。此书称一号为「头号」。
